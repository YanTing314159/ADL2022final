...
from transformers import default_data_collator, Trainer, BlenderbotForConditionalGeneration, BlenderbotTokenizer, T5ForConditionalGeneration
...
class OTTerDataset(torch.utils.data.Dataset):
...
def read_data(data_dir):
    ...
    datasets = {}
    for split in splits:
        datasets[split] = load_dataset('csv', data_files=os.path.join(data_dir, 'text.csv'))
        if split != 'test':
...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_root", default='./data', type=str, help="training data dir"
    )
    parser.add_argument(
        "--model_name_or_path", default="facebook/blenderbot-400M-distill", type=str, help="model to finetune"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    tokenizer.pad_token = tokenizer.eos_token
    train_dataset = read_data(args.dataset_root) 
    #train_dataset = load_dataset("blended_skill_talk", split="train")
    model = BlenderbotForConditionalGeneration.from_pretrained(args.model_name_or_path).to(device)
